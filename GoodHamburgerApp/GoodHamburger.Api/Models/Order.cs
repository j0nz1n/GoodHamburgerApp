...
namespace GoodHamburger.Api.Models
{
    public class Order
    {
        ...
        {
            if (OrderItems.Any(oi => oi.MenuItemId == menu.MenuItemId))
                throw new Exception($"O item {menu.Name} já está no pedido.");

            var item = new OrderItem
            {
                // REMOVA a linha Id = Guid.NewGuid() se o banco estiver configurado para gerar automático
                // Ou garanta que o OrderId seja atribuído corretamente:
